...
class BaselineDroneSimulator:
    ...
    def __init__(self, spec_path: str):
        ...
        self._drag_linear = np.asarray(self.spec.aero.drag_linear, dtype=np.float64).reshape(3)
        self._rotor_radius = 0.5 * float(self.spec.propeller.diameter)
        # Battery sag (T1.B): loaded Kv approximation V_nom -> omega_max.
        # Current model: I_per_motor = K_t * omega (back-EMF style). Voltage
        # drops under load via R_loss (battery R_int + ESC + motor windings
        # lumped together, ~3x spec R_int). Hover-current baseline subtracted
        # so the analytic hover throttle still produces exactly mg — keeps
        # T0.2 (1 cm/5 s drift tolerance) within fp noise.
        self._V_nominal = float(self.spec.battery.voltage_nominal)
        self._R_int = float(self.spec.battery.internal_resistance)
        self._K_v = self._omega_max / self._V_nominal if self._V_nominal > 0 else 0.0
        self._K_t = 1.0 / self._K_v if self._K_v > 0 else 0.0
        self._R_loss = 3.0 * self._R_int
        _omega_hover = self._hover_omega()
        self._I_hover_baseline = self._K_t * self.spec.num_motors * _omega_hover
    ...
    def step(
        ...
        for _ in range(n_sub):
            self._integrate_substep(
                motor_cmds, ext_force_world, ext_torque_world, sub_dt, ext_ground_z
            )
    ...
    def _integrate_substep(
        ...
        sub_dt: float,
        ext_ground_z: Optional[float],
    ) -> None:
        # 1. Battery sag: per-motor current I = K_t * omega (back-EMF style).
        # Subtract hover baseline so no sag at hover (preserves T0.2); excess
        # current above hover drops voltage via lumped system resistance.
        I_total = float(self._K_t * np.sum(self._motor_omegas))
        I_excess = max(0.0, I_total - self._I_hover_baseline)
        V_bat = self._V_nominal - self._R_loss * I_excess
        omega_max_eff = self._K_v * max(V_bat, 0.0)

        # 2. Motor RPM dynamics (first-order lag toward throttle * dynamic omega_max)
        self._motor_dynamics_step(motor_cmds, sub_dt, omega_max_eff)

        # 2. Body-frame force and torque from current motor RPMs.
        # Ground-effect: amplify per-motor thrust by Cheng-Frantz approximation
        # T_eff = T * (1 + a*(R/h)^2)  for h < 4R  (R = rotor radius).
        thrust_mult = 1.0
        if ext_ground_z is not None:
            h = self._position[2] - ext_ground_z
            if h > 1e-3 and h < 4.0 * self._rotor_radius:
                thrust_mult = 1.0 + 0.05 * (self._rotor_radius / h) ** 2
        F_body, tau_body = self._compute_motor_wrench(thrust_mult)
        ...
        F_world = R @ F_body
        # Linear aero drag in body frame (coefficients are per body axis).
        v_body = R.T @ self._velocity
        F_world += R @ (-self._drag_linear * v_body)
    ...
    # ------------------------------------------------------------------ internals
    def _motor_dynamics_step(
        self, motor_cmds: np.ndarray, dt: float, omega_max: float
    ) -> None:
        ...
        """
        omega_cmd = motor_cmds * omega_max
    ...
    def _compute_motor_wrench(self, thrust_multiplier: float = 1.0):
        ...
        T = self._k_T * omega2 * thrust_multiplier        # (N,) thrust magnitudes
