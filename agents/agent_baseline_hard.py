...
class BoatKalmanFilter:
    ...
    def __init__(
        ...
        self.min_gate_m = float(min_gate_m)
        ...
        self.last_rejected_measurement: Optional[np.ndarray] = None
        self.last_innovation_norm = 0.0
        self.last_gate_radius = float("inf")
    ...
    def update(self, z: np.ndarray) -> bool:
        ...
        if not self.initialized:
            ...
            self.last_rejected_measurement = None
            self.last_innovation_norm = 0.0
            self.last_gate_radius = float("inf")
            self.consecutive_rejections = 0
            return True
        ...
        gate_radius = max(
            self.min_gate_m,
            self.gate_sigma * float(np.sqrt(np.max(np.diag(S)))),
        )
        self.last_innovation_norm = innovation_norm
        self.last_gate_radius = gate_radius
        if (
            innovation_norm > gate_radius
            and self.consecutive_rejections < self.max_consecutive_rejections
        ):
            # Measurement disagrees too strongly with the predicted boat
            # state. Keep the prediction for short spikes, but do not lock
            # ourselves out forever: after a few consistent rejects, the
            # next measurement is accepted as reacquisition.
            self.last_rejected_measurement = z.copy()
...
class BaselineAgent:
    ...
    def __init__(self, drone_spec: Optional[DroneSpec] = None):
        ...
        self.pid_x = PID(kp=0.17, ki=0.02, kd=0.40)
        self.pid_y = PID(kp=0.15, ki=0.02, kd=0.40)
        self.pid_z = PID(kp=0.19, ki=0.02, kd=0.30)
    ...
    def estimate(self, perception: Dict, drone_state: Dict, history) -> Dict:
        ...
        if perception.get("detected", False):
            ...
            accepted = self.kf.update(marker_world)
            if not accepted:
                ...
                return {
                    ...
                    "innovation_norm": self.kf.last_innovation_norm,
                    "gate_radius": self.kf.last_gate_radius,
                    # Rejected rvec/tvec should not drive deck-level logic.
                    "boat_tilt": None,
